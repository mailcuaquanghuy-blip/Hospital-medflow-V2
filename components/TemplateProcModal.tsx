...
export const TemplateProcModal: React.FC<TemplateProcModalProps> = ({
  ...
  const eligibleMainStaff = useMemo(() => staff.filter(s => s.deptId === currentDept.id && s.mainCapabilityIds?.includes(formData.procedureId || '')), [staff, currentDept, formData.procedureId]);
  const eligibleAssistants = useMemo(() => staff.filter(s => s.deptId === currentDept.id && s.assistantCapabilityIds?.includes(formData.procedureId || '')), [staff, currentDept, formData.procedureId]);
