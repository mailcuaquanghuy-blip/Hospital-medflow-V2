...
import { MachineShift, Procedure, Staff, Department, Appointment, AttendanceRecord, AttendanceStatus } from '../types';
  ...
  const handleSave = () => {
    ...
    const checkStaff = (sId: string, roleName: string) => {
      ...
      if (att && att.status !== 'PRESENT') {
        const isMorning = startTime <= '11:30';
            ...
            <div className="p-6 overflow-y-auto flex-1 flex flex-col gap-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-2">
                  ...
                  <input type="time" lang="vi-VN" value={startTime} onChange={e => setStartTime(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all" />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Giờ kết thúc</label>
                  <input type="time" lang="vi-VN" value={endTime} onChange={e => setEndTime(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all" />
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <select value={staffId} onChange={e => setStaffId(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all">
                  ...
                  {staff.filter(s => s.deptId === currentDept.id && s.mainCapabilityIds.includes(procedureId) && s.id !== assistant1Id && s.id !== assistant2Id && !attendanceRecords.some(r => r.staffId === s.id && r.date === activeDate && r.status === AttendanceStatus.OFF_FULL)).map(s => (
              ...
              {requiresAssistant1 && (
                <div className="flex flex-col gap-2">
                  ...
                  <select value={assistant1Id} onChange={e => setAssistant1Id(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all">
                    ...
                    {staff.filter(s => s.deptId === currentDept.id && s.assistantCapabilityIds.includes(procedureId) && s.id !== staffId && s.id !== assistant2Id && !attendanceRecords.some(r => r.staffId === s.id && r.date === activeDate && r.status === AttendanceStatus.OFF_FULL)).map(s => (
                      <option key={s.id} value={s.id}>{s.name} ({s.role})</option>
                    ))}
                  </select>
                </div>
              ...
              {requiresAssistant2 && (
                <div className="flex flex-col gap-2">
                  ...
                  <select value={assistant2Id} onChange={e => setAssistant2Id(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all">
                    ...
                    {staff.filter(s => s.deptId === currentDept.id && s.assistantCapabilityIds.includes(procedureId) && s.id !== staffId && s.id !== assistant1Id && !attendanceRecords.some(r => r.staffId === s.id && r.date === activeDate && r.status === AttendanceStatus.OFF_FULL)).map(s => (
                      <option key={s.id} value={s.id}>{s.name} ({s.role})</option>
                    ))}
                  </select>
                </div>
