...
export const checkConflict = (
  ...
  if (patientId && procedureId) {
    ...
    const proc = getProcFromCache(procedures, procedureId);
    if (currentPatient && proc) {
      ...
      if (procDept && procDept.type === DepartmentType.SUPPORT) {
        ...
        if (!referral) {
          ...
          const alreadyExists = existingAppointments.some(appt => 
            appt.patientId === patientId &&
            appt.procedureId === procedureId &&
            appt.date === newDate &&
  ...
  const checkStaffAttendance = (id: string, name: string, role: string) => {
    ...
    if (isHoliday) {
      ...
      if (attendance) {
        ...
        const morningEndMin = timeStringToMinutes(shift1.end);
  ...
  const dayAppointments = getDayAppointmentsFromCache(existingAppointments, newDate);
  ...
  if (currentProc?.requireMachine) {
    if (currentProc.availableMachines?.length) {
      ...
      if (assignedMachineId) {
         ...
         // Find a machine that fits - Prefer least busy machines for the day
         let bestMachine: string | undefined;
         let minTotalAppts = Infinity;
         ...
         for (const machineCode of currentProc.availableMachines) {
            ...
            if (capacity > 1) {
               ...
               if (overlappingCount < capacity) hasSpaceAtTime = true;
            } else {
               ...
               if (!hasOverlap) hasSpaceAtTime = true;
            }

            if (hasSpaceAtTime) {
               const totalApptsCount = machineApps.length;
               if (totalApptsCount < minTotalAppts) {
                  minTotalAppts = totalApptsCount;
                  bestMachine = machineCode;
                  if (totalApptsCount === 0) break; // Found an empty machine
               }
            }
         }

         if (bestMachine) {
            assignedMachineId = bestMachine;
         } else {
  ...
  if (currentProc?.isPreRequisite && patientId) {
    ...
    const existingBlockingInDept = dayAppointments.find(appt => {
      if (appt.id === excludeAppointmentId || appt.patientId !== patientId) return false;
    ...
    const hasPriorInDept = dayAppointments.some(appt => {
      if (appt.id === excludeAppointmentId || appt.patientId !== patientId) return false;
      const apptProc = getProcFromCache(procedures, appt.procedureId);
  ...
  if (currentProc?.isPostRequisite && patientId) {
    const hasSubsequentInDept = dayAppointments.some(appt => {
      if (appt.id === excludeAppointmentId || appt.patientId !== patientId) return false;
      const apptProc = getProcFromCache(procedures, appt.procedureId);
      if (apptProc?.deptId !== currentProc.deptId || apptProc?.isIndependent) return false;
      const apptStart = timeStringToMinutes(appt.startTime);
  ...
  const hasScheduleConflict = uniqueConflicts.some(c => 
    c.level === 1 && 
