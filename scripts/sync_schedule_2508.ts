...
async function runSync2508(dryRun = false) {
  ...
  // Map active patients in dept_lao on 2026-08-25
  const patientMap = new Map<string, any>();
  const activePatients25 = allPatients.filter(p => {
  ...
  for (const p of activePatients25.filter(p => p.admittedByDeptId === 'dept_lao')) {
    patientMap.set(norm(p.name), p);
  }
  // Then any active patient
  for (const p of activePatients25) {
  ...
  for (const s of allStaff) {
    // Prefer Doctor record if multiple
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const apptId = `appt_20260825_${String(i + 1).padStart(3, '0')}`;
  ...
  console.log(`Total final appointments on 2026-08-25: ${finalAppointments.length}`);
  ...
  // 3. Clear existing appointments for 2026-08-25 in Supabase
  ...
  console.log(`Successfully inserted all ${finalAppointments.length} appointments for 2026-08-25 into Supabase!`);

  // 5. Also sync to Firestore
  try {
    const firebaseConfig = JSON.parse(fs.readFileSync('./firebase-applet-config.json', 'utf-8'));
    const fApp = initializeApp(firebaseConfig, 'syncApp25');
    ...
    // Delete existing on 25/8 in Firestore
    ...
    if (fsSnap.size > 0) {
      ...
      await batch.commit();
      console.log('Cleared Firestore 2026-08-25 appointments.');
    }
  ...
  const { data: verifyRows, error: vErr } = await supabase
    ...
    .filter('data->>date', 'eq', TARGET_DATE);
  if (vErr) throw vErr;
  console.log(`Verification: Total appointments on 2026-08-25 now in Supabase = ${verifyRows?.length}`);

  console.log(`--- FINISHED SYNC FOR 2026-08-25 ---`);
}

// Check CLI arguments for --dry-run
const isDryRun = process.argv.includes('--dry-run');
runSync2508(isDryRun).catch(err => {
