...
async function run() {
  ...
  const XOA_BOP_PROC_ID = "pr_rj91ghjep"; // Xoa bóp procedure ID
  ...
  if (sourceAppts.length === 0) {
    console.error(`No appointments found on ${sourceDate} for Geriatrics. Please double check.`);
    process.exit(1);
  }
  ...
  let skippedXoaBopCount = 0;

  for (const appt of sourceAppts) {
    // Check if it is a Xoa bóp appointment of Bùi Thị Thu Hà
    const isXoaBop = appt.procedureId === XOA_BOP_PROC_ID;
    const involvesHa = appt.staffId === HA_ID || appt.assistant1Id === HA_ID || appt.assistant2Id === HA_ID;

    if (isXoaBop && involvesHa) {
      skippedXoaBopCount++;
      const patientName = patients.find(p => p.id === appt.patientId)?.name || "Unknown";
      console.log(`Skipping copy of Xoa bóp appointment for patient "${patientName}" (ID: ${appt.id}) because it involves Bùi Thị Thu Hà.`);
      continue;
    }

    // Determine the map targets for staff: Replace Bùi Thị Thu Hà with Lê Hương Giang
    let mappedStaffId = appt.staffId;
    if (mappedStaffId === HA_ID) mappedStaffId = GIANG_ID;

    let mappedAsst1Id = appt.assistant1Id;
    if (mappedAsst1Id === HA_ID) mappedAsst1Id = GIANG_ID;

    let mappedAsst2Id = appt.assistant2Id;
    if (mappedAsst2Id === HA_ID) mappedAsst2Id = GIANG_ID;
    ...
    // Standardize a beautiful, collision-free new ID for target date
  ...
  console.log(`Skipped ${skippedXoaBopCount} Xoa bóp appointments involving Bùi Thị Thu Hà.`);
  ...
  for (const appt of mappedAppointmentsBeforeWriting) {
    ...
    console.log(`Saved Appt ${successCount}/${mappedAppointmentsBeforeWriting.length} - BN: "${patientName}", TT: "${procName}" (${appt.startTime} - ${appt.endTime}). Máy: ${appt.assignedMachineId || "-"}. Xung đột: ${conflictRes.hasConflict}`);
  }
